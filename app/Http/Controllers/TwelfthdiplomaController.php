...
class TwelfthdiplomaController extends Controller
{
    ...
    public function getFormData(Request $request)
    {
        ...
        $current_step = DB::table('form_statuses')->where('user_id', $user->id)
            ->value('form_step');
        $matriculation_year = DB::table('matriculation')->where('user_id', $user->id)
            ->value('year_of_passing');
        $category = TwelfthDiplomaCategory::updateOrCreate(['user_id' => $user->id], ['urn' => $user->username, 'user_id' => $user->id, 'category' => $request->category]);
        if ($request->category == "XII" || $request->category == "both")
        {
            ...
            if ($twelfth_file)
            {
                ...
                $twelfth_details = Twelfth::updateOrCreate(['user_id' => $user->id], ['user_id' => $user->id, 'file' => '/documents/twelfth/' . $twelfth_file_url]);
            }
            $twelfth_details = Twelfth::updateOrCreate(['user_id' => $user->id], ['urn' => $user->username, 'user_id' => $user->id, 'pincode' => $request->pincode_12, 'city' => $request->city_12, 'jee_rank' => $request->jee_rank_12, 'state' => $request->state_12,
            //'marks_type' => $request->marks_type_12,
            'maximum_marks' => $request->maximum_marks_12, 'obtained_marks' => $request->obtained_marks_12, 'percentage' => ($request->obtained_marks_12) * 100 / ($request->maximum_marks_12) , 'institution_name' => $request->institution_name_12, 'board' => $request->board_12, 'year_of_passing' => $request->year_of_passing_12, 'year_gap' => 2016 - $matriculation_year,
    ...
    {
        $user = auth()->user();
        $category = DB::table('twelfth_diploma_category')->where('user_id', $user->id)
            ->first();
        if (!$category)
        {
        ...
        {
            $twelfth = DB::table('twelfth')->where('user_id', $user->id)
                ->first();
            $diploma = DB::table('diploma')->where('user_id', $user->id)
                ->first();
            return response()
                ->json(["category" => $category->category, "twelfth" => $twelfth, "diploma" => $diploma]);
        }
        elseif ($category->category == "XII")
        {
            $twelfth = DB::table('twelfth')->where('user_id', $user->id)
                ->first();
            return response()
                ->json(["category" => $category->category, "twelfth" => $twelfth]);
        }
        elseif ($category->category == "diploma")
        {
            $diploma = DB::table('diploma')->where('user_id', $user->id)
                ->first();
            return response()
                ->json(["category" => $category->category, "diploma" => $diploma]);
        }
    }
}